...
def create_degree_granting_institution(source_record: ET.Element, context: Context) -> ET.Element:
    ...
    organisation_old_id = source_record.findtext(".//grantingInstitution/organisationId")
    print(f"Organisation old ID: {organisation_old_id}")
    if organisation_old_id is not None:
        cora_id = get_cora_id_by_old_id(
            organisation_old_id, record_type=DIVA_ORGANISATION_RECORD_TYPE, context=context
        )
        organisation = create_record_link_using_name_type_id(
            name_in_data="organisation", record_type=DIVA_ORGANISATION_RECORD_TYPE, record_id=cora_id
        )

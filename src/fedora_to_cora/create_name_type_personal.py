...
def create_name_type_personals(source_record: ET.Element) -> list[ET.Element]:
    """
        ...
        for person in persons:
            if person is not None:
                name_type_personals.append(
                    create_name_type_personal(person, [role_term], repeat_id)
                )
                repeat_id += 1
    ...
    for contributor in source_record.findall(".//otherContributors/contributor"):
        ...
        name_type_personals.append(
            create_name_type_personal(contributor, role_terms, repeat_id)
        )
        repeat_id += 1
...
def create_name_type_personal(
    person: ET.Element, role_terms: list[str], repeatId: int
) -> ET.Element:
    """
    ...
    for i, organisation in enumerate(person.findall("./organisations/organisation")):
        name_type_personal.append(create_affiliation(organisation, i))

    return name_type_personal


def create_affiliation(organisation: ET.Element, repeat_id: int) -> ET.Element:
    controlled = organisation.find("./controlled")
    if controlled is not None and controlled.text == "true":
        return create_affiliation_for_controlled_organisation(organisation, repeat_id)
    else:

...
def create_related_item_type_journal(source_record: ET.Element, context: Context) -> ET.Element:
    ...
    if journal_old_id is not None:
        ...
        journal = create_record_link_using_name_type_id(
            name_in_data="journal", record_type=DIVA_JOURNAL_RECORD_TYPE, record_id=cora_id
        )
...
def _create_identifiers(source_record: ET.Element, related_item: ET.Element) -> ET.Element | None:
    pissn = create_identifier(
        source_record=source_record, type="issn", source_selector="./journal/printedIssn"
    )
    pissn.set("displayLabel", "pissn")
    append_if_value(related_item, pissn)

    eissn = create_identifier(
        source_record=source_record, type="issn", source_selector="./journal/electronicIssn"
    )

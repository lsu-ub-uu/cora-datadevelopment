...
def create_related_item_type_proceeding(
    ...
    related_item = ET.Element("relatedItem", type="proceeding", otherType="text")
...
def _create_title_info(source_record: ET.Element) -> ET.Element | None:
    ...
    if source_title:
        ET.SubElement(title_info, "title").text = clean_rich_text(source_title)

    source_subtitle = proceedings_title.findtext("./subTitle")
    if source_subtitle:
        ET.SubElement(title_info, "subtitle").text = clean_rich_text(source_subtitle)
    return title_info

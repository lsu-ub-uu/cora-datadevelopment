...
urlSpecificDomain = 'https://cora.diva-portal.org/diva/rest/record/searchResult/publicOrganisationSearch?searchData={"name":"search","children":[{"name":"include","children":[{"name":"includePart","children":[{"name":"divaOrganisationDomainSearchTerm","value":"nordiskamuseet"}]}]}]}'
...
def start():
    ...
    for record in json_data["dataList"]["data"]:
        recordChildren = record["record"]["data"][
            "children"
        ]  # listar alla barn till record
        recordInfoChild = cora_data.find_child_with_name_in_data(
            recordChildren, "recordInfo"
        )  # hittar recordInfo
        recordInfoChildren = recordInfoChild[
            "children"
        ]  # listar alla barn till recordInfo
        validationType = cora_data.getValidationTypeLink(recordInfoChildren)
        domain = cora_data.get_first_atomic_value_with_name_in_data(
            recordInfoChildren, "domain"
        )
        if (
            domain != "ntnu"
            and domain != "hhs"
            and domain != "hkr"
            and validationType != "rootOrganisation"
        ):
            workorder = buildRecordToCreateAndValidate(
                domain, recordChildren, recordInfoChildren
            )  # skapar workorder och record
            newRecord = workorder["record"]  # plockar ut record
            createdCoraRecordResponseText = createRecord(
                newRecord
            )  # create record, makes a request
            ...
            # print(workorder)
            #            print(newRecord)
            relationOldNewIds, linksToEarlierId, linksToParentId = storeIdData(
                recordChildren, createdCoraRecordResponseText
            )
...
def buildRecordToCreateAndValidate(domain, recordChildren, recordInfoChildren):
    ...
    cora_data.append_value_to_list(
        newDomain,
        {
            "children": [
                {"name": "linkedRecordType", "value": "permissionUnit"},
                {"name": "linkedRecordId", "value": newDomain},
            ],
            "name": "permissionUnit",
        },
        recordInfoList,
    )
    id = cora_data.get_first_atomic_value_with_name_in_data(recordInfoChildren, "id")
    cora_data.append_value_to_list(id, {"name": "oldId", "value": id}, recordInfoList)
    validationType = cora_data.getValidationTypeLink(recordInfoChildren)
    ...
    cora_data.append_value_to_list(
        newValidationType,
        {
            "children": [
                ...
                {"name": "linkedRecordId", "value": newValidationType},
            ],
            "name": "validationType",
        },
        recordInfoList,
    ...
    # parts of record
    namePartSwe = cora_data.getOrganisationNameValueWithNameInData(
        recordChildren, "organisationName"
    )
    cora_data.append_value_to_list(
        namePartSwe,
        {
            "children": [
                {
                    "children": [{"name": "namePart", "value": namePartSwe}],
                    "name": "name",
    ...
    namePartEng = cora_data.getOrganisationNameValueWithNameInData(
        recordChildren, "organisationAlternativeName"
    )
    cora_data.append_value_to_list(
        namePartEng,
        {
            "children": [
                {
                    "children": [{"name": "namePart", "value": namePartEng}],
                    "name": "name",
                    "attributes": {"type": "corporate"},
                }
            ],
    ...
    url = cora_data.get_first_atomic_value_with_name_in_data(recordChildren, "URL")
    cora_data.append_value_to_list(
        url,
        {"children": [{"name": "url", "value": url}], "name": "location"},
        recordBodyList,
    )
    orgNum = cora_data.get_first_atomic_value_with_name_in_data(
        recordChildren, "organisationNumber"
    )
    cora_data.append_value_to_list(
        orgNum,
        {
            "name": "identifier",
            "attributes": {"type": "organisationNumber"},
            "value": orgNum,
        },
        recordBodyList,
    )
    orgCode = cora_data.get_first_atomic_value_with_name_in_data(
        recordChildren, "organisationCode"
    )
    cora_data.append_value_to_list(
        orgCode,
        {
            "name": "identifier",
            "attributes": {"type": "organisationCode"},
            "value": orgCode,
        },
        recordBodyList,
    )
    endDate = cora_data.get_first_atomic_value_with_name_in_data(
        recordChildren, "closedDate"
    )
    ...
    adressChild = cora_data.find_child_with_name_in_data(recordChildren, "address")
    if adressChild is not None:
        adressChildren = adressChild["children"]  # listar alla barn till address
        postOfficeBox = cora_data.get_first_atomic_value_with_name_in_data(
            adressChildren, "box"
        )
        cora_data.append_value_to_list(
            postOfficeBox,
            {"name": "postOfficeBox", "value": postOfficeBox},
            newAddressList,
        )
        street = cora_data.get_first_atomic_value_with_name_in_data(
            adressChildren, "street"
        )
        cora_data.append_value_to_list(
            street, {"name": "street", "value": street}, newAddressList
        )
        postcode = cora_data.get_first_atomic_value_with_name_in_data(
            adressChildren, "postcode"
        )
        cora_data.append_value_to_list(
            postcode, {"name": "postcode", "value": postcode}, newAddressList
        )
        place = cora_data.get_first_atomic_value_with_name_in_data(
            adressChildren, "city"
        )
        cora_data.append_value_to_list(
            place, {"name": "place", "value": place}, newAddressList
        )
        country = cora_data.get_first_atomic_value_with_name_in_data(
            adressChildren, "country"
        )
        newCountry = checkCountryAndSetNewValue(country)
        cora_data.append_value_to_list(
            newCountry, {"name": "country", "value": newCountry}, newAddressList
        )
    cora_data.append_value_to_list(
        adressChild, {"children": newAddressList, "name": "address"}, recordBodyList
    )
...
def storeIdData(recordChildren, createdCoraRecordResponseText):
    ...
    coraRecordRecordInfoChild = cora_data.find_child_with_name_in_data(
        coraRecordChildren, "recordInfo"
    )
    coraRecordRecordInfoChildren = coraRecordRecordInfoChild[
        "children"
    ]  # borde kuna slås ihop ovan och använda en av befintliga funktionerna istället
    newId = cora_data.get_first_atomic_value_with_name_in_data(
        coraRecordRecordInfoChildren, "id"
    )
    oldId = cora_data.get_first_atomic_value_with_name_in_data(
        coraRecordRecordInfoChildren, "oldId"
    )
    earlierLinkOldId = cora_data.getParentEarlierLinks(
        recordChildren, "earlierOrganisation"
    )
    parentLinkOldId = cora_data.getParentEarlierLinks(
        recordChildren, "parentOrganisation"
    )
...
def openValidationOrderBaseFile():
    validation_order_baseFile = r"data/cora/validate/validation_order_base.json"
    with open(validation_order_baseFile, "r") as openfile:
...
def removeActionLinksFromRecord(recordToClean):
    ...
    recordInfo = cora_data.find_child_with_name_in_data(recordChildren, "recordInfo")
    permissionUnit = cora_data.find_child_with_name_in_data(
        recordInfo["children"], "permissionUnit"
    )
    del permissionUnit["actionLinks"]
    validationType = cora_data.find_child_with_name_in_data(
        recordInfo["children"], "validationType"
    )
    del validationType["actionLinks"]
    dataDivider = cora_data.find_child_with_name_in_data(
        recordInfo["children"], "dataDivider"
    )
    del dataDivider["actionLinks"]
    type = cora_data.find_child_with_name_in_data(recordInfo["children"], "type")
    del type["actionLinks"]
    createdBy = cora_data.find_child_with_name_in_data(
        recordInfo["children"], "createdBy"
    )
    del createdBy["actionLinks"]
    updated = cora_data.find_child_with_name_in_data(recordInfo["children"], "updated")
    updatedBy = cora_data.find_child_with_name_in_data(updated["children"], "updatedBy")
...
base_url = {
    "minikube": "http://minikube:30982/rest/record/",
...
def checkValidationTypeLinkAndGetNewValue(
    validationType,
):  # behöver skippa posten om den har rot.

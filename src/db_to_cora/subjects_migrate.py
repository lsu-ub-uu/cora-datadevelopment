...
def _migrate_subject(
    ...
    transformed_subject = transform_subject(source_record)
    result = create_record(
        transformed_subject, record_type="diva-subject", context=context
    )
...
def migrate_subjects(
    ...
    with (
        Pool(
            processes=min(len(classic_subjects), processes),
        ...
        tqdm(total=len(classic_subjects), desc="Importing records") as progress,
    ):
        for status, record_mapping in pool.imap_unordered(
            _migrate_subject, classic_subjects
        ):
            counts[status] += 1

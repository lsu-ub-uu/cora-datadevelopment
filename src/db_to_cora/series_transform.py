...
def transform_series(source_record: ET.Element) -> ET.Element:
    ...
    append_if_value(series, _create_origin_info(source_record, origin_type="originInfo"))
    append_if_value(series, _create_identifiers_from_source_with_type_issn(source_record, identifier_type = "pissn"))
    append_if_value(series, _create_identifiers_from_source_with_type_issn(source_record, identifier_type = "eissn"))
    ...
    append_if_value(series, _create_record_link_using_name_type_id(source_record, name_in_data = "organisation", record_type = "diva-organisation"))
    append_if_value(series, _create_record_link_test(source_record, link_repeat_id, record_type = "diva-series", name_in_data = "related", related_type="host"))
    append_if_value(series, _create_record_link_test(source_record, link_repeat_id, record_type = "diva-series", name_in_data = "related", related_type="preceding"))
...
def _create_title_info_type_alternative(source_record: ET.Element) -> ET.Element:
    title = source_record.find(f".//alternative_title")
    subtitle = source_record.find(f".//alterantive_subtitle")
    if title is not None and title.text:
        return create_title_info_type_alternative(
            title.text, subtitle
            )
...
def _create_genre(source_record: ET.Element, genre_repeat_id: dict) -> ET.Element | None:
    publication_type = source_record.find(f".//publication_type_id")
    if publication_type is not None and publication_type.text:
        return create_genre(
            publication_type.text, genre_repeat_id
            )
...
def _create_record_link_using_name_type_id(source_record: ET.Element, name_in_data: str, record_type: str)-> ET.Element | None:

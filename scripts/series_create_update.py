...
def new_record_build(data_record):
    ...
    common_data.record_info_build(
        name_in_data, permission_unit, data_record, new_record_element
    )
    common_data.title_info_build(data_record, new_record_element)
    common_data.titleInfo_alternative_build(
        data_record, new_record_element, "alternative"
    )
    ...
    counter = common_data.identifier_build(
        data_record, new_record_element, "pissn", counter
    )
    counter = common_data.identifier_build(
        data_record, new_record_element, "eissn", counter
    )
    counter = common_data.genre_build(
        data_record, new_record_element, publication_map, counter
    )
...
def validate_record(data_record):
    ...
    newRecordToValidate = common_data.validateRecord_build(
        record_type, file_path_validate_base, newRecordToCreate
    )
...
def create_record(data_record):
    ...
    relationOldNewIds, linksToPrecedingIds, linksToHostIds = store_ids(
        data_record, response.text
    )
...
def loop_id_lists(relationOldNewIds, linksToPrecedingIds, linksToHostIds):
    ...
    for oldId, newId in relationOldNewIds.items():
        if newId in linksToPrecedingIds or newId in linksToHostIds:
            ...
            cleanedRecord = common_data.remove_actionLinks_from_record(
                newRecord, name_in_data
            )
            if newId in linksToHostIds:
                parentOldId = linksToHostIds[newId]
                parentNewId = relationOldNewIds[parentOldId]
                related_subject_build(
                    record_type, cleanedRecord, "host", repeatId, parentNewId
                )
                repeatId += 1
            if newId in linksToPrecedingIds:
                ...
                for earlierOldId in earlierOldIds:
                    ...
                    related_subject_build(
                        record_type, cleanedRecord, "preceding", repeatId, earlierNewId
                    )
                    repeatId += 1
...
def related_subject_build(record_type, cleanedRecord, relatedType, counter, value):
    related = ET.SubElement(
        cleanedRecord, "related", repeatId=str(counter), type=relatedType
    )
...
def orgLink_build(new_record_element, data_record):
    ...
    if orgOldIdFromSource is not None and orgOldIdFromSource.text:
        ...
        if idToLink is not None and idToLink.text:
            ...
            ET.SubElement(relatedOrganisation, "linkedRecordType").text = (
                "diva-organisation"
            )
...
def search_query_org_oldId(orgOldId):
    ...
    oldId_search_query = (
        'searchData={"name":"search","children":[{"name":"include","children":[{"name":"includePart","children":[{"name":"oldIdSearchTerm","value":"'
        + orgOldId
        + '"}]}]}]}'
    )
    search_url = (
        constants.BASE_URL[system]
        + "searchResult/diva-organisationSearch?"
        + oldId_search_query
    )
